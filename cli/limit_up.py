...
from src.mining.limit_up_evolution import LimitUpEvolutionEngine, describe_genome
...
def evolve_cmd(
    ...
    table = Table(title="涨停结构因子 Top 10（锁定测试集不参与选优）")
    table.add_column("因子", style="cyan")
    table.add_column("来源")
    table.add_column("结构公式", max_width=38)
    table.add_column("适用板数")
    table.add_column("训练均值/胜率", justify="right")
    table.add_column("验证均值/胜率", justify="right")
    table.add_column("测试均值/胜率", justify="right")
    table.add_column("测试盈亏比", justify="right")
    table.add_column("结论")
    for item in outcome.evaluations[:10]:
        genome = item.genome
        table.add_row(
            genome.name,
            genome.source,
            describe_genome(genome),
            f"{genome.min_board}-{genome.max_board}",
            f"{item.train.avg_return:+.2f}%/{item.train.win_rate:.0%}",
            f"{item.validation.avg_return:+.2f}%/{item.validation.win_rate:.0%}",
            f"{item.test.avg_return:+.2f}%/{item.test.win_rate:.0%}",
            f"{item.test.pnl_ratio:.2f}",
            "[green]可操作[/green]" if item.accepted else "[yellow]研究中[/yellow]",
        )
    console.print(table)
    best = outcome.best
    if best:
