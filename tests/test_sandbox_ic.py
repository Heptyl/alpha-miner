...
def db_with_data(tmp_path):
    ...
    # 插入 15 个交易日的价格数据，10 只股票，以容纳 development/holdout 边界。
    codes = [f"00000{i}" for i in range(10)]
    dates = pd.bdate_range(end="2024-06-14", periods=15).strftime("%Y-%m-%d").tolist()
...
'''
        result = sandbox.evaluate(code, lookback_days=15, as_of=datetime(2024, 6, 14))
    ...
    def test_no_limit_param_in_db_query(self):
        ...
        try:
            ...
            assert "limit=" not in content or "limit=None" in content, \
                "sandbox.py 中有对 db.query 的 limit= 调用，会报错"
        except FileNotFoundError:
            pass  # 文件不存在则跳过

    def test_runner_uses_shared_pit_backtester(self):
        """独立 runner 必须复用主进程的 PIT 编译器和回测器。"""
        runner_path = "src/mining/_sandbox_runner.py"
        try:
            with open(runner_path, encoding="utf-8") as f:
                content = f.read()
            assert "compile_compute_source" in content
            assert "FactorBacktester" in content
        except FileNotFoundError:
