...
重要：执行器已预注入 pd、np、datetime、timedelta、re；生成代码禁止 import。
直接写函数定义和逻辑即可。
...
def compute(universe, as_of, db):
    ...
    Args:
        ...
        db: 已绑定 decision_at 的 PointInTimeView，只提供 query/query_range
...
```python
# 参数化查询；as_of 不得晚于执行器绑定的 decision_at
...
只允许以上两个结构化读取方法。禁止 execute、原始 SQL、连接/路径访问、写入、
bypass_snapshot、backtest_mode、open/eval/exec 和任何 import。
...
6. 不要 import；所需安全对象已预注入
7. RETRO_DEVELOPMENT 结果只能用于开发搜索，不能用于 PAPER 或验收
