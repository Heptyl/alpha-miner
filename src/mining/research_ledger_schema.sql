...
CREATE TABLE IF NOT EXISTS research_evidence (
 ...
 event_type TEXT NOT NULL CHECK(event_type IN ('DEVELOPMENT_RESULT','HOLDOUT_OPENED','HOLDOUT_RESULT')),
...
CREATE TRIGGER IF NOT EXISTS trg_result_requires_open BEFORE INSERT ON research_evidence WHEN NEW.event_type='HOLDOUT_RESULT' AND NOT EXISTS(SELECT 1 FROM research_evidence WHERE candidate_hash=NEW.candidate_hash AND event_type='HOLDOUT_OPENED') BEGIN SELECT RAISE(ABORT,'holdout result requires opened event'); END;
